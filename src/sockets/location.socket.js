...
module.exports = (io) => {
    ...
    io.on('connection', (socket) => {
        console.log(`🔌 [Socket.IO] Cliente conectado: ${socket.id} (Rol: ${socket.user.rol})`);
        ...
        if (socket.user.rol === 'ADMIN' || socket.user.rol === 'SUPERVISOR') {
            ...
            io.to('dashboard_room').emit('user_status_changed', { id_user: socket.user.id, status: 'ONLINE' });
        }
        ...
        socket.on('update_location', async (data) => {
            try {
                ...
                await geofenceService.checkLocationAgainstGeofences(userId, data.latitud, data.longitud, timestamp, io);
