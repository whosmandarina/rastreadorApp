  ...
  try {
    ...
    for (const loc of locations) {
      ...
      if (vel < 2) {
        if (!posibleParada) {
          posibleParada = {
            start: loc.timestamp_captura,
            end: loc.timestamp_captura,
            lat: loc.latitud,
            lng: loc.longitud,
          };
        } else {
        ...
        if (posibleParada) {
          const dur =
            (new Date(posibleParada.end) - new Date(posibleParada.start)) /
            60000;
          if (dur >= 3)
            paradas.push({
              ...posibleParada,
              duracion_minutos: dur.toFixed(1),
            });
          posibleParada = null;
        }
      }
    ...
    if (posibleParada) {
      const dur =
        (new Date(posibleParada.end) - new Date(posibleParada.start)) / 60000;
      if (dur >= 3)
        paradas.push({ ...posibleParada, duracion_minutos: dur.toFixed(1) });
    }

    const velocidad_promedio = speedCount > 0 ? totalSpeed / speedCount : 0;
    const tiempo_total_parado = paradas.reduce(
      (acc, p) => acc + parseFloat(p.duracion_minutos),
      0,
    );
...
exports.exportPDF = async (req, res) => {
  try {
    ...
    const [users] = await db.query(
      'SELECT nombre, correo FROM Users WHERE id_user = ?',
      [userId],
    );
    const user = users[0];
    if (!user)
    ...
    const velocidadPromedio =
      speedCount > 0 ? (totalSpeed / speedCount).toFixed(1) : '0.0';
    const tiempoParado = paradas
      .reduce((a, p) => a + parseFloat(p.duracion_minutos), 0)
      .toFixed(1);
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Reporte_${user.nombre.replace(/ /g, '_')}.pdf`,
    );
    doc.pipe(res);
    ...
    if (paradas.length > 0) {
      ...
      paradas.forEach((p, i) => {
        doc
          ...
          .fillColor('#3a5a6e')
          .text(
            `${i + 1}.  Inicio: ${fmtDate(p.start)}   →   Fin: ${fmtDate(p.end)}   (${p.duracion_minutos} min)`,
          );
      });
      doc.moveDown(1.5);
    }
    ...
    if (locations.length === 0) {
      ...
      if (locations.length > 200) {
        ...
        doc
          ...
          .fillColor('#6b8a9a')
          .text(
            `* Se muestran los primeros 200 de ${locations.length} registros. Descarga el Excel para el historial completo.`,
          );
      }
    }
    ...
    doc
      ...
      .font('Helvetica')
      .text(
        `Generado el ${fmtDate(new Date())} (hora de México) — Sistema de Rastreo`,
        { align: 'center' },
      );
...
exports.exportExcel = async (req, res) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=Ruta_${userId}.xlsx`,
    );
