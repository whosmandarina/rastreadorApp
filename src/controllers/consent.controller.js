...
exports.getConsentByUser = async (req, res) => {
  try {
    ...
    if (req.user.rol === 'USER' && req.user.id !== parseInt(id_user)) {
      return res
        .status(403)
        .json({ message: 'No puedes ver el consentimiento de otro usuario' });
    }
    ...
    if (consents.length === 0) {
      return res.status(404).json({
        message:
          'No se encontraron registros de consentimiento para este usuario',
        hasConsent: false,
      });
    }
...
exports.revokeConsent = async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM Consents WHERE id_user = ?', [userId]);
