...
exports.syncOfflineLocations = async (req, res) => {
  try {
    ...
    if (!locations || !Array.isArray(locations) || locations.length === 0) {
      return res
        .status(400)
        .json({ message: 'Se requiere un array de ubicaciones' });
    }

    const sortedLocations = locations.sort(
      (a, b) => new Date(a.timestamp_captura) - new Date(b.timestamp_captura),
    );
    ...
    try {
      ...
      res.json({
        ...
        puntos_guardados: sortedLocations.length,
      });
    } catch (error) {
    ...
    res
      .status(500)
      .json({ message: 'Error en el servidor al sincronizar ubicaciones' });
  }
};
