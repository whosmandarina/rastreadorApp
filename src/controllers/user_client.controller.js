...
exports.assignUserToClient = async (req, res) => {
    try {
        ...
        if (!id_user || !id_client) {
            return res.status(400).json({ message: 'id_user e id_client son requeridos' });
        }

        // Evitar duplicados
        const [existing] = await db.query('SELECT * FROM User_Client WHERE id_user = ? AND id_client = ?', [id_user, id_client]);
        if (existing.length > 0) {
            return res.status(400).json({ message: 'El usuario ya está asignado a este cliente' });
        }

        await db.query('INSERT INTO User_Client (id_user, id_client) VALUES (?, ?)', [id_user, id_client]);
        res.status(201).json({ message: 'Usuario asignado al cliente exitosamente' });
    } catch (error) {
...
exports.removeAssignment = async (req, res) => {
    try {
        ...
        const [result] = await db.query('DELETE FROM User_Client WHERE id_user = ? AND id_client = ?', [id_user, id_client]);
