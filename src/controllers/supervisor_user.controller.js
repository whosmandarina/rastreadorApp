...
exports.assignSupervisorToUser = async (req, res) => {
    try {
        ...
        if (!id_supervisor || !id_user) {
            return res.status(400).json({ message: 'id_supervisor e id_user son requeridos' });
        }

        // Evitar duplicados
        const [existing] = await db.query('SELECT * FROM Supervisor_User WHERE id_supervisor = ? AND id_user = ?', [id_supervisor, id_user]);
        if (existing.length > 0) {
            return res.status(400).json({ message: 'El usuario ya está asignado a este supervisor' });
        }

        await db.query('INSERT INTO Supervisor_User (id_supervisor, id_user) VALUES (?, ?)', [id_supervisor, id_user]);
        res.status(201).json({ message: 'Usuario asignado al supervisor exitosamente' });
    } catch (error) {
...
exports.getUsersBySupervisor = async (req, res) => {
    try {
        ...
        if (req.user.rol === 'SUPERVISOR' && req.user.id !== parseInt(id_supervisor)) {
            return res.status(403).json({ message: 'No tienes permiso para ver los usuarios de otro supervisor' });
        }
...
exports.removeAssignment = async (req, res) => {
    try {
        ...
        const [result] = await db.query('DELETE FROM Supervisor_User WHERE id_supervisor = ? AND id_user = ?', [id_supervisor, id_user]);
