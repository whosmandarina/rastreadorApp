...
exports.register = async (req, res) => {
  try {
    ...
    const { nombre, correo, telefono, password, codigo_supervisor } =
      parsed.data;
    ...
    if (supervisors.length === 0) {
      return res
        .status(400)
        .json({ message: 'Código de supervisor no válido' });
    }
    ...
    const [result] = await db.query(
      ...
      [nombre, normalizedEmail, telefono || null, hashedPassword, 'USER'],
    );
...
exports.login = async (req, res) => {
  try {
    ...
    const { correo, password, device_id } = parsed.data;
    const normalizedEmail = correo.trim().toLowerCase();

    // Buscar usuario
    const [users] = await db.query(
      'SELECT * FROM Users WHERE correo = ? AND is_active = TRUE',
      [normalizedEmail],
    );
    const user = users[0];
    ...
    if (user.rol === 'USER') {
      await db.query(
        'UPDATE Sessions SET is_active = FALSE WHERE id_user = ? AND is_active = TRUE',
        [user.id_user],
      );
    }
...
exports.logout = async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE Sessions SET is_active = FALSE WHERE token_jti = ?',
      [jti],
    );
