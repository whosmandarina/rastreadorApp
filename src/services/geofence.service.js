...
exports.checkLocationAgainstGeofences = async (userId, lat, lng, timestamp, io) => {
    try {
        ...
        for (const geofence of geofences) {
            ...
            if (geofence.tipo === 'CIRCLE') {
                ...
                const formattedPolygon = polygon.map(p => ({ latitude: p.lat, longitude: p.lng }));
                isInside = geolib.isPointInPolygon({ latitude: lat, longitude: lng }, formattedPolygon);
            }
            ...
            let lastEventType = lastEventRows.length > 0 ? lastEventRows[0].tipo_evento : null;
            ...
            if (newEventType) {
                ...
                const alertType = newEventType === 'ENTER' ? 'GEOFENCE_ENTER' : 'GEOFENCE_EXIT';
