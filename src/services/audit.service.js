...
const logAction = (logData) => {
    ...
    if (!logData.action_type || !logData.details) {
        console.error('AUDIT LOG ERROR: Faltan action_type o details para registrar la auditoría.');
        return;
    }

    const { id_user_action, action_type, target_entity, target_id, details } = logData;

    db.query(
        'INSERT INTO Audit_Logs (id_user_action, action_type, target_entity, target_id, details) VALUES (?, ?, ?, ?, ?)',
        [id_user_action || null, action_type, target_entity || null, target_id || null, details]
    ).catch(err => {
